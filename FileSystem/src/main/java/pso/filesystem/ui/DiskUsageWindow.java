...
public class DiskUsageWindow extends JFrame {
    ...
    public DiskUsageWindow(FileSystem fileSystem) {
        super("Uso del disco — " + fileSystem.bootBlock().volumeName());
    ...
    private JPanel buildHeader() {
        ...
        panel.add(usageBar, BorderLayout.SOUTH);
        return panel;
    }
    ...
    private JPanel buildLegend() {
        ...
        for (int i = 0; i < TYPE_NAMES.length; i++)
            panel.add(legendItem(BLOCK_FILL[i], BLOCK_BORDER_COLOR[i], TYPE_NAMES[i]));
        return panel;
    }
    ...
    private JPanel legendItem(Color fill, Color border, String label) {
        ...
        JPanel sw = new JPanel() {
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.setColor(fill);   g2.fillRoundRect(0, 1, 10, 10, 2, 2);
    ...
    private class UsageBar extends JPanel {
        ...
        UsageBar() { setPreferredSize(new Dimension(0, 6)); setOpaque(false); }
        void setRatio(double r) { this.ratio = r; repaint(); }
        @Override protected void paintComponent(Graphics g) {
            Graphics2D g2 = (Graphics2D) g;
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            int w = getWidth(), h = getHeight();
            g2.setColor(BORDER_COLOR); g2.fillRoundRect(0, 0, w, h, h, h);
            int filled = (int) (w * ratio);
            if (filled > 0) {
                Color c = ratio > 0.85 ? new Color(226, 75, 74)
                        : ratio > 0.65 ? new Color(239, 159, 39) : ACCENT;
                g2.setColor(c); g2.fillRoundRect(0, 0, filled, h, h, h);
            }
        }
    }
    ...
    private class GridPanel extends JPanel {
        ...
        @Override public Dimension getPreferredSize() {
            ...
            return new Dimension(
                GRID_PAD * 2 + cols * (BLOCK_PX + BLOCK_GAP) - BLOCK_GAP,
                Math.max(GRID_PAD * 2 + rows * (BLOCK_PX + BLOCK_GAP) - BLOCK_GAP, 200)
            );
        }
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            for (int i = 0; i < totalBlocks; i++) {
                ...
                if (i == selectedBlock) { g2.setColor(new Color(30,30,30)); g2.setStroke(new BasicStroke(1.2f)); }
                else { g2.setColor(BLOCK_BORDER_COLOR[t]); g2.setStroke(new BasicStroke(0.5f)); }
    ...
    private class DetailPanel extends JPanel {
        ...
        DetailPanel() {
            ...
            add(swatch); add(Box.createVerticalStrut(10));
            add(titleLabel); add(Box.createVerticalStrut(4));
            add(typeLabel);  add(Box.createVerticalStrut(12));
            add(fieldRow("Inodo", inodeLabel)); add(Box.createVerticalStrut(6));
            add(fieldRow("Dueño", ownerLabel)); add(Box.createVerticalGlue());
            add(hint);
        ...
        void clear() {
            ...
            inodeLabel.setText("—"); ownerLabel.setText("—");
            repaint();
        }
    ...
    private static void styleScrollbar(JScrollPane sp) {
        sp.getVerticalScrollBar().setUI(new javax.swing.plaf.basic.BasicScrollBarUI() {
            @Override protected void configureScrollBarColors() {
                thumbColor = new Color(200, 200, 200); trackColor = BG;
            }
